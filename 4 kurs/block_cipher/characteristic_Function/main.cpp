...
int main()
{
    // PART 1
    // SM4
    uint8_t pi[256] = {0xd6, 0x90, 0xe9, 0xfe, 0xcc, 0xe1, 0x3d, 0xb7, 0x16, 0xb6, 0x14, 0xc2, 0x28, 0xfb, 0x2c, 0x05,
    ...
    // kuznhechik
    // uint8_t pi[256] = {252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77,
    ...
    Function<uint8_t, uint8_t> fx(8, 8, pi);
    fx.setData();
    printf("\np(pi)*2^8 =     %f", fx.p_f * (1 << 8));
    printf("\ndelta(pi)*2^8 = %f", fx.delta_f * (1 << 8));
    printf("\nlambda(pi) = %d\n", lambda(pi));
}
