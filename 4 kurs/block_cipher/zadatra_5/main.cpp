...
int main()
{
    ...
    for (int r = 0; r < 16; r++)
    {
        for (int c = 0; c < 16; c++)
        {
            L_phi[16 * r + c] = (int)(16 * PI.delta[r][c]);
        }
    }
    printMatrix_16(L_phi);
                        ...
                        for (b2[1] = 0; b2[1] < 16; b2[1]++)
                            ...
                            for (b2[3] = 0; b2[3] < 16; b2[3]++)
                                ...
                                for (b3[1] = 0; b3[1] < 16; b3[1]++)
                                    ...
                                    {
                                        // printf("b1= (%x%x%x%x)\n", b1[0], b1[1], b1[2], b1[3]);
                                        if ((L_phi[a1[0] * 16 + b1[0]] == 0) || (L_phi[a1[1] * 16 + b1[1]] == 0) ||
                                            (L_phi[a1[2] * 16 + b1[2]] == 0) || (L_phi[a1[3] * 16 + b1[3]] == 0))
                                        {
                                            continue;
                                        }
                                        ////////////////
                                        assign(a2, b1);
                                        P(a2);
                                        if ((L_phi[a2[0] * 16 + b2[0]] == 0) || (L_phi[a2[1] * 16 + b2[1]] == 0) ||
                                            (L_phi[a2[2] * 16 + b2[2]] == 0) || (L_phi[a2[3] * 16 + b2[3]] == 0))
                                        {
                                            continue;
                                        }
                                        ////////////////
                                        assign(a3, b2);
                                        P(a3);
                                        if ((L_phi[a3[0] * 16 + b3[0]] == 0) || (L_phi[a3[1] * 16 + b3[1]] == 0) ||
                                            (L_phi[a3[2] * 16 + b3[2]] == 0) || (L_phi[a3[3] * 16 + b3[3]] == 0))
                                        {
                                            continue;
                                        }
                                        ///////////////
                                        assign(a4, b3);
                                        P(a4);
                                        float res = (float)(L_phi[a1[0] * 16 + b1[0]] * L_phi[a1[1] * 16 + b1[1]] * L_phi[a1[2] * 16 + b1[2]] * L_phi[a1[3] * 16 + b1[3]]); // / 16 / 16 / 16 / 16;
                                        // printf("1:  %f\n", res);
                                        res *= (float)(L_phi[a2[0] * 16 + b2[0]] * L_phi[a2[1] * 16 + b2[1]] * L_phi[a2[2] * 16 + b2[2]] * L_phi[a2[3] * 16 + b2[3]]) / 16 / 16 / 16 / 16;
                                        // printf("2:  %f\n", res);
                                        res *= (float)(L_phi[a3[0] * 16 + b3[0]] * L_phi[a3[1] * 16 + b3[1]] * L_phi[a3[2] * 16 + b3[2]] * L_phi[a3[3] * 16 + b3[3]]) / 16 / 16 / 16 / 16;
                                        if (res > 4000) // |res| > 4000
                                        {
